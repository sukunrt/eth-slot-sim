...
- `DecoupledParams` fields are copied one-by-one into `r.finalityRoundSize` / `r.fcVoteOffset` /
...
| AC slots per finality slot | `AC_SLOTS_PER_FINALITY_SLOT`, "k" | `ac_slots_per_finality_slot` | `AcSlotsPerFinalitySlot`, `DecoupledParams.K`, `r.finalityRoundSize` | — |
